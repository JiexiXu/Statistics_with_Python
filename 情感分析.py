...
discfile = r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\hair_dryer.tsv'
...
with open(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\big.txt', 'r') as f:
    WORDS = tokens(f.read())
...
stopwords = set()
fr = open(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\python词典\stopwords.txt','r',encoding='utf-8')
for word in fr:
    stopwords.add(word.strip())#Python strip() 方法用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列。
    # #读取否定词文件
    not_word_file = open(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\python词典\合集.txt','r+',encoding='utf-8')
    not_word_list = not_word_file.readlines()
    not_word_list = [w.strip() for w in not_word_list]
    #生成新的停用词表
    with open(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\python词典\stopwords_new.txt','w',encoding='utf-8') as f:
        for word in stopwords:
            if word not in not_word_list:
                f.write(word+'\n')
stopwords_new=set()
fn=open(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\python词典\stopwords_new.txt','r',encoding='utf-8')
for word in fn:
...
#生成否定词字典，赋值-1
not_word_df = pd.read_csv(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\python词典\否定词.txt', sep=";", names=['vocabulary'])
not_word_df['value'] = -1
not_dict = dict(zip(not_word_df['vocabulary'],not_word_df['value']))

#生成程度副词字典（针对六种程度副词，分别赋权）
degree_word_df = pd.read_csv(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\python词典\程度副词.txt', sep=";", names=['vocabulary'])
...
#读取senticnet文件，导入数据
sen_df = pd.read_excel(r'D:\高雪辰\数学学院与经济学院\公共课、选修课\python统计分析\小组展示\senticnet\senticnet_list.xlsx',header=0,usecols='A:B')
